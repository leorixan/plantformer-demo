...
func physics_update(delta: float) -> void:
	# Up-dash has its own SuperWallJump. Other wall contact remains normal wall jump.
	var wall_direction := player.get_wall_direction()
	if player.has_jump_buffer() and wall_direction != 0:
		if player.dash_direction == Vector2.UP:
			player.do_super_wall_jump(-wall_direction)
		else:
			player.do_wall_jump(-wall_direction)
		state_machine.transition_to("Air")
		return
	# Move first: down-diagonal landing facts belong to this dash frame, never prior is_on_floor().
	var dash_ended := player.update_dash(delta)
