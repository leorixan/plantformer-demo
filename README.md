...
目前灰盒阶段，全是纯色块，没有美术和音效，真正的关卡也还没做，能玩的只有一个练技巧的测试房。

我大三在读，软件工程，这是第一个认真做的游戏项目。
...
而且坏了往往当场察觉不到 —— 等你发现 Ultra 触发不了，可能是三天前改抓墙的时候埋下的。
...
这是**学习性质的复刻**，来源我不藏着：移动参数和帧序都照 Celeste 作者公开的 `Player.cs`
...
这两个常量在 `scripts/player/player.gd:12-17`，理由我写在注释里了 ——
不写的话过两个月自己看着像脏代码，一"整理"就又坏一遍。

**4. 我的第一版测试是假的**

它直接调函数，断言的又是我刚写下的那几个数，所以随便怎么改都能过。
后来发现游戏明明已经坏了它还全绿，整个推掉重写成真实物理驱动。
现在加完一个功能，我会先故意把某个参数或分支改坏，看着对应断言变红了，才敢信这条测试有用。
...
- 相机是硬跟随，没平滑、没前瞻、没有房间边界，一冲刺就晃。
- 测试只覆盖数值，覆盖不了手感。除我之外没人玩过，那些高级技巧对没练过的人顺不顺手，我心里没底。
